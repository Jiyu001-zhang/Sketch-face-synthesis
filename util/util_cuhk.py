...
def xyandpix(mapping_face, mapping_hair):
    ...
    # 取出噪点
    face_filtered_dict = {key: value for key, value in face_pixels_dict.items() if value > 0.5}  # 根据阈值过滤
    ...
    # 将注意力值高的点多采一次
    double_face = [tup_face for tup_face in face_random_elements if tup_face[1] > 0.9]
    double_face = []
    # 合并
    ...
    double_hair = [tup_hair for tup_hair in hair_random_elements if tup_hair[1] > 0.7]
    double_hair = []
