...
def xyandpix(mapping_face, mapping_hair):
    ...
    # 取出噪点
    face_filtered_dict = {key: value for key, value in face_pixels_dict.items() if value > 0.2}  # 根据阈值过滤
    hair_filtered_dict = {key: value for key, value in hair_pixels_dict.items() if value > 0.2}  # 根据阈值过滤
    # random_point_256=[]
    if len(hair_filtered_dict) < 128:
        face_random_elements = random.sample(face_filtered_dict.items(), 384)
        double_face = [tup_face for tup_face in face_random_elements if tup_face[1] > 0.35]
        face = [tup_face_point[0] for tup_face_point in face_random_elements + double_face]
        random_point_256 = face
    else:
        face_random_elements = random.sample(face_filtered_dict.items(), 256)
        double_face = [tup_face for tup_face in face_random_elements if tup_face[1] > 0.33]
        face = [tup_face_point[0] for tup_face_point in face_random_elements + double_face]

        hair_random_elements = random.sample(hair_filtered_dict.items(), 128)
        double_hair = [tup_hair for tup_hair in hair_random_elements if tup_hair[1] > 0.3]
